...
| Memory | 跨请求会话记忆、InMemory 存储、窗口裁剪、摘要、Runtime/Starter 集成 |
| Examples | 默认验证入口输出 `All 11 verification scenarios passed.` |
...
完成 v0.2 后，业务系统应能够用统一方式注册工具、运行 Agent、控制权限、追踪调用链，并基于同一 `sessionId` 跨请求加载会话记忆。后续继续扩展 Skill、Task 和 Multi-Agent。
